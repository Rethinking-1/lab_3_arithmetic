...
class Lexema
{
  ...
  std::string val;
public:
  Lexema(std::string _val = "", int priority = 0) :type(priority), val(_val) {}
  std::string getVal() { return val; }
  virtual int gettype() { return type; }
};

class Formula /*:protected Lexema*/
{
  ...
  double Ans;
  int sizeFormula;
  std::vector<Lexema>LexDefForm;
  //std::vector<Lexema>LexPolish;
  int CheckPriority(std::string str);
  int CountErrorsbrackets();
  bool CheckCorrectOperation();
  std::string FindProhibitedOperations();
public:
  Formula(std::string formula = "");
  void PrintLexem();
  bool FormulaChecker();
  bool Split_into_Lexems();
  std::string FormulaConverter();
  bool Calculate_Polish();
  double Get_Ans();
  std::string Get_PolishForm() { FormulaConverter(); return PolishForm; }
};
...
inline bool Formula::Calculate_Polish()
{
  FormulaChecker();
  FormulaConverter();
  Split_into_Lexems();
